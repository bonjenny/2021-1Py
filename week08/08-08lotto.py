# -*- coding: utf-8 -*-
"""
Created on Tue May  4 20:12:42 2021
...
rank = { 6: ('1등', 100_000000),     # 1등 10억 원, 맞춘 번호 6
         5: {1: ('2등', 50_000000),  # 2등 5천만 원, 맞춘 번호 5개 + 보너스
             0: ('3등', 10_000000)}, # 3등 1백만 원, 맞춘 번호 5개
         4: ('4등', 50000),          # 4등 5만 원, 맞춘 번호 4개
         3: ('5등', 5000)            # 5등 5천 원, 맞춘 번호 3개
        }
...
def getwinner(lotto):
    ...
    for i, item in enumerate(lotto):
        print('%c' % (ord('A') + i), end = ' ')
        win = winnum.intersection(item) # 당첨 번호 구하
        if win:
            wincnt = len(win)
