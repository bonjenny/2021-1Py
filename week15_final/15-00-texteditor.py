...
class TextEditor():
    ...
    def make_menu(self):
        ...
        # 메뉴 아이템 Find - 수정됨
        imenu = Menu(self.menubar, tearoff=0)
        imenu.add_command(label="FindText", command=self.find_text, accelerator="Ctrl+F") # 수정됨
        # 메뉴바에 Find를 추가
        self.menubar.add_cascade(label="Find", menu=imenu)
    ...
    def find_text(self, event=None): # 수정됨
        ft = Toplevel(root)
        ft.geometry("200x85+600+200")
        ft.title("Find Text")
        
        Label(ft, text="Enter the word want to find", width=26).grid(row=0, column=0, padx=5, pady=3)
        # 찾을 단어 입력
        fent = Entry(ft, width=26)
        fent.grid(row=1, column=0, padx=5, pady=3)
        
        def find():
            srchstr = fent.get().strip()
            print(srchstr)
            all = self.editor.get(0.0, END)
            tmpline = all.splitlines()
            print(tmpline)
            i = 1
            c = 0
            for tmp in tmpline:
                s = int(tmp.find(srchstr))
                if s == -1:
                    break
                l = s + int(len(srchstr))
                x = float(str(i)+'.'+str(s))
                y = float(str(i)+'.'+str(l))
                print(x, y)
                self.editor.tag_add(str(c), str(x), str(y))
                self.editor.tag_config(str(c), background="yellow")
                i += 1
                c += 1
        
        fbtn = Button(ft, text="Find", command=find)
        fbtn.grid(row=2, column=0, columnspan=2, padx=5, pady=3)
    ...
    def bind_events(self, event=None):
        ...
        self.editor.bind("<Control-f>", self.find_text) # 수정됨
        self.editor.bind("<Control-F>", self.find_text) # 수정됨
