# -*- coding: utf-8 -*-
"""
Created on Fri May 21 11:27:49 2021
...
def add(): # 1 증가시키며 레이블의 문자열에 지정
    n = int(count.get())
    n += 1
    count.set(n)

def sub(): # 1 감소시키며 레이블의 문자열에 지정
    n = int(count.get())
    n -= 1
    count.set(n)
...
bsub = Button(win, text=str[1], command = sub) # 누르면 함수 add() 호출
